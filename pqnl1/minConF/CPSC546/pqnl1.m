function [x,r,g,info] = pqnl1( A, b, tau, sigma, x, options )
...
while 1
    ...
    if  testUpdateTau
        try
            ...
            [x iterpqn f g r funvalues xNorm1part rNorm2part timePqnProject] = ...
                minConF_PQN(funObj,xOld,funProj,opt,iter,options.iterations,weights,decTol,sigma);
            clear funvalues;
            xNorm1(iter+1:iter+iterpqn-1) = xNorm1part;
            rNorm2(iter+1:iter+iterpqn-1) = rNorm2part;   
            iter = iter+iterpqn-1;
            timeProject = timeProject+timePqnProject;
            %x(abs(x) < opt.optTol) = 0;
