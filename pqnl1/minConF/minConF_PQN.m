function [x,itnpqn,f,g,r,funEvals,xNorm1part,rNorm2part,timePqnProject,projects] = minConF_PQN(funObj,x,funProj,options,itn,maxitn,weights,decTol,sigma)
...
while funEvals <= maxIter
    ...
    % Check optimality
    if i == itn+2; 1;% do not check this for the first itn
    else
        if optCond < optTol
        ...
        testUpdateTau  = ((testRelChange1 && rNorm2part(i-1-itn) >  2 * sigma) || ...
            (testRelChange2 && rNorm2part(i-1-itn) <= 2 * sigma));
...
xNorm1part(itnpqn:end) = [];
rNorm2part(itnpqn:end) = [];


% in case PQNl1 failed, project to gradient direction instead
if i == itn+1;
   % spg 
end
end
