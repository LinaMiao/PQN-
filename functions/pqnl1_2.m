function [x,r,g,info] = pqnl1_2( A, b, tau, sigma, x, options, sigma_ref)
%
% [x, r, g, info] = pqnl1(A, b, tau, sigma, x0, options)
%
% ---------------------------------------------------------------------
...
%   [x,r,g,info] = pqnl1(A, b, 0, 1e-3, [], opts); % Find BP sol'n.
%
% AUTHORS
% =======
...
printf('\n');
printf(' %s\n',repmat('=',1,82));
printf(' PQNL1_SLIM v.%s (%s) based on v.1017\n', REVISION, DATE);
printf(' %s\n',repmat('=',1,82));
...
while 1
    ...
    try
        ...
        opt.fid = options.fid;
        opt.primal_norm = options.primal_norm;
        opt.dual_norm = options.dual_norm;
        ...
        [x,iterpqn,f,g,r,xNorm1part,rNorm2part,timePqnProject,testUpdateTau,nLine] = ...
            minConF_PQN_pqnl1(funObj,xOld,funProj,opt,iter,options.iterations,weights,decTol,sigma,b,tau,bpTol,sigma_ref);
...
% % threshold of the final solution;
% x(abs(x)<=optTol) = 0;
...
switch (stat)
   ...
   case EXIT_LESS_SIGMA_REF
      printf('\n EXIT -- residual less than sigma ref\n');
   otherwise
