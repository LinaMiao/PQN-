function [x,f,funEvals,projects] = minConf_SPG(funObj,x,funProj,options,decTol,flag)
...
while funEvals <= maxIter
    ...
    if flag
        testRelChange1 = (abs(f - f_old) <= abs(decTol * f));
        testRelChange2 = (abs(f - f_old) <= 1e-1 * abs(f) * (abs(norm(f,2) - optTol)));
        testUpdate  = ((testRelChange1 && norm(f) >  2 * optTol) || ...
            (testRelChange2 && norm(f) <= 2 * optTol));


        if testUpdate
            fprintf('break of testUpdate\n');
            break; % break of testUpdate
        end
    end
