function [x,itnpqn,f,g,r,xNorm1part,rNorm2part,timePqnProject,testUpdateTau,nLineTot] = minConF_PQN_pqnl1(funObj,x,funProj,options,itn,maxitn,weights,decTol,sigma,b,tau,bpTol,sigma_ref)
% 
% this code is here only to see what's the performance if pqn has 10
% corrections, 
%
...
[fid,verbose,numDiff,optTol,maxIter,maxProject,suffDec,corrections,adjustStep,bbInit,SPGoptTol,SPGiters,SPGtestOpt] = ...
    myProcessOptions(...
    options,'fid',1,'verbose',2,'numDiff',0,'optTol',1e-12,'maxIter',100,'maxProject',100000,'suffDec',1e-4,...
    'corrections',10,'adjustStep',1,'bbInit',1,'SPGoptTol',1e-12,'SPGiters',500,'SPGtestOpt',1);

% Output Parameter Settings
if verbose >= 3
   fprintf(fid,'Running PQN...\n');
   fprintf(fid,'Number of L-BFGS Corrections to store: %d\n',corrections);
   fprintf(fid,'Spectral initialization of SPG: %d\n',bbInit);
   fprintf(fid,'Maximum number of SPG iterations: %d\n',SPGiters);
   fprintf(fid,'SPG optimality tolerance: %.2e\n',SPGoptTol);
   fprintf(fid,'PQN optimality tolerance: %.2e\n',optTol);
   fprintf(fid,'Quadratic initialization of line search: %d\n',adjustStep);
   fprintf(fid,'Maximum number of function evaluations: %d\n',maxIter);
   fprintf(fid,'Maximum number of projections: %d\n',maxProject);
end

% Output Log
if verbose >= 2
        fprintf(fid,'\n');
        fprintf(fid,'Inside of minConf_PQN \n');
        fprintf(fid,'%10s %10s %10s %15s %15s %15s\n','Iteration','FunEvals','Projections','Step Length','rNorm2','Opt Cond');
end
...
if sum(abs(funProj(x-g)-x)) < min(optTol^2/2,1e-12)
    if verbose >= 1
        fprintf(fid,'First-Order Optimality Conditions Below optTol at Initial Point\n');
    end
    testUpdateTau = 1;
    return;
end
...
while funEvals <= maxIter
        ...
        if itnpqn == 0 
            ...
            % Solve Sub-problem
            tStart = toc;
            %fprintf(fid,'begin solving sub-problem for i = %3.0f\n',i)
            [p,subProjects] = solveSubProblem(x,g,HvFunc,funProj,SPGoptTol,SPGiters,SPGtestOpt,feasibleInit,x,r);
            projects = projects+subProjects;
            timePqnProject = timePqnProject + toc-tStart;
            %fprintf(fid,'finish solving sub-problem for i = %3.0f\n',i)
        end
    ...
    if gtd > -min(optTol^2/2,1e-12)
        if verbose >= 1
            fprintf(fid,'Directional Derivative below optTol\n');
        end
...
%       or, quit this round of hessian approxmation
   ...
   % Backtracking Line Search
    f_old = f;
     while f_new > f + suffDec*t*gtd || ~isLegal(f_new)
         ...
         if ~isLegal(f_new) || ~isLegal(g_new)
             if verbose == 3
                 fprintf(fid,'Halving Step Size\n');
             end
             t = t/2;
         else
             if verbose == 3
                 fprintf(fid,'Cubic Backtracking\n');
             end
         ...
         if t < temp*1e-3
             if verbose == 3
                 fprintf(fid,'Interpolated value too small, Adjusting\n');
             end
             t = temp*1e-3;
         elseif t > temp*0.6
             if verbose == 3
                 fprintf(fid,'Interpolated value too large, Adjusting\n');
             end
             t = temp*0.6;
         end
         ...
         if sum(abs(t*d)) < min(optTol^2/2,1e-12) || t == 0
             if verbose == 3
                 fprintf(fid,'Line Search failed\n');
             end
             t = 0;
             f_new = f;
             g_new = g;
    ...
    dNorm   = undist(options.dual_norm(d,weights)); 
    dNorm   = gNorm; % as original spgl1
    rNorm   = norm(r, 2);
    bNorm   = norm(b,2);
    ...
    if isempty(x)
        ...
        xNorm1part(i-itn) = options.primal_norm(x,weights);
    end
    rNorm2part(i-itn) = norm(r,2);
    
     % Output Log
    if verbose >= 2
            fprintf(fid,'%10d %10d %10d %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,projects,t,rNorm,optCond);
    end
    
    % Check optimality
    
    if optCond < min(optTol^2/2,1e-12)
        fprintf(fid,'First-Order Optimality Conditions Below optTol\n');
        break;
    end
    
    if sum(abs(t*d)) < min(optTol^2/2,1e-12)
        if verbose >= 1
            fprintf(fid,'Step size below optTol\n');
        end
        break;
    end
    
    if norm(r,2) <= sigma_ref
        fprintf(fid,'rNorm less than sigma_ref\n');
        break;
        %funEvals = maxIter + 1;
    end
    ...
    if ~isempty(sigma)
        ...
        testRelChange1 = (abs(f - f_old) <= (30) * decTol * f);
        testRelChange2 = (abs(f - f_old) <= 1e0 * f * (abs(norm(r,2) - sigma)));
...
%         end
        ...
        if testUpdateTau
            fprintf(fid,'break of testUpdateTau\n');
    ...
    if abs(f-f_old) < min(optTol^2/2,1e-12) %% Lina :: optTol
        if verbose >= 1
            fprintf(fid,'Function value changing by less than optTol\n');
        end
        testUpdateTau = 1;
        break;
    end
    ...
    if funEvals*funEvalMultiplier > maxIter
        if verbose >= 1
            fprintf(fid,'Function Evaluations exceeds maxIter\n');
        end
        testUpdateTau = 1;
        break;
    end
    ...
    if projects > maxProject
        if verbose >= 1
            fprintf(fid,'Number of projections exceeds maxProject\n');
        end
        testUpdateTau = 1;
        break;
    end
    ...
    if norm(r,2) <= optTol;  %% Lina :: optTol
        if verbose >= 1
            fprintf(fid,'Optimal solution found\n');
        end
        break;
    end
...
function [p,subProjects] = solveSubProblem(x,g,H,funProj,optTol,maxIter,testOpt,feasibleInit,x_init,r)
...
%   gts      = scale * (g' * s);
    ...
    if debug
       fprintf(fid,' LS %2i  %13.7e  %13.7e  %13.6e  %8.1e\n',...
