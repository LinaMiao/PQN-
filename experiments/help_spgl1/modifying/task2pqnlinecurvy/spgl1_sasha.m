function [x,r,g,info] = spgl1_sasha(A, b, tau, sigma, x, options, params)
...
'lineSrchIt' ,     10 , ... % Maximum number of line search iterations for spgLineCurvy, originally 10 ...   %%%Lina:: 215
...
'funPenalty' , @funLS          , ... % default penalty - backward compatible with spgl1                      %%%Lina:: not exist 
'proxy'      ,      0          , ... % advanced option that computes pareto curve in a user-specified way.   %%%Lina:: not exist 
'restore'    ,      0          , ... % whether to restore best previous answer. for large problems, don't want to do this. %%%Lina:: not exist 
'PQN'        ,      0            ... % whether use PQN method or not ...
 );
...
maxLineErrors = Inf;     % Maximum number of line-search failures (DISABLED)   %%% Lina :: moved from 245
...
printTau      = false;
nNewton       = 0;
bNorm         = funPenalty(b, params); %%% Lina :: 257
stat          = false;
timeProject   = 0;
timeMatProd   = 0;
...
if isempty(x)
    ...
    x         = project(x,tau);
    r         = b - funForward(x, [], params);  % r = b - f(x) %%% Lina :: why nProdA increase 1
    nProdA = nProdA + 1;
    [f g g2]     = funCompositeR(r, funForward, funPenalty, params);
...
lastFv(1) = f;  %%% Lina :: if this kept, does that mean non monotone PQN?
fBest     = f;
xBest     = x;
fOld      = f;
...
while 1
    ...
    rNorm   = f;  % rNorm and f are exactly the same.    %%% Lina :: what happened here? why not rNorm = norm(r,2);
    ...
    if isempty(x)  %%% Lina nnzX overwritten here
        nnzX    = 0;
    else
        nnzIter = nnzIter + 1;
    ...
    % Multiple tau: Check if found root and/or if tau needs updating.
       ...
       if options.PQN == 1 %%% Lina :: do not test this outside minConfPQN if PQN
       else
       ...
       if testUpdateTau
          ...
          if tau < tauOld
             ...
             x = project(x,tau);   %%% Lina :: different here, guess not matteres a lot
    ...
    % Update history info
    if options.PQN == 1 %%% Lina :: if using PQN update later in try loop
    else
        if isempty(x)
            xNorm1(iter+1) = 0;
        else
    ...
    if options.PQN == 1 %%% Lina :: if PQN, update g here
        gOld = g;
        try
...
function [fObj gObj rObj] = funPQN(x,funForward,funPenalty)
    if isempty(x)
         rObj = b; % r = b - Ax
