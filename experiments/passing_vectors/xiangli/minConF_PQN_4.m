function [x,itnpqn,f,g,r,funEvals,xNorm1part,rNorm2part,timePqnProject,projects,S,Y,g_old,x_old,Hdiag] = minConF_PQN_4(funObj,x,funProj,options,itn,maxitn,weights,decTol,sigma,b,tau,bpTol,S,Y,g_old,x_old,Hdiag)
% 
% this version only put minConF_PQN insised spgl1, 
% without any passing of Hessian vectors
% with all the exit of PQN kept
% this version, set the spg optimal criteria same with the one working for
% help spgl1

% the difference here is to test optimal criteria even for the first PQN
% iteration
...
%             end
    ...
    % Check optimality
    if i == inf; 
        1;% which will never happen, so check for all is
    else
        if optCond < optTol
