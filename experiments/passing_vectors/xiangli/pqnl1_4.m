function [x,r,g,info] = pqnl1_4( A, b, tau, sigma, x, options )
%
    ...
    if  testUpdateTau
        try
            ...
            [x,iterpqn,f,g,r,~,xNorm1part,rNorm2part,timePqnProject,~,S,Y,g_old,x_old,Hdiag] = ...
                minConF_PQN_4(funObj,xOld,funProj,opt,iter,options.iterations,weights,decTol,sigma,b,tau,bpTol,S,Y,g_old,x_old,Hdiag);
            if iterpqn<=1 1;
            else
