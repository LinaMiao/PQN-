function [x,itnpqn,f,g,r,funEvals,xNorm1part,rNorm2part,timePqnProject,projects,S,Y,g_old,x_old,Hdiag] = minConF_PQN(funObj,x,funProj,options,itn,maxitn,weights,decTol,sigma,b,tau,bpTol,S,Y,g_old,x_old,Hdiag,passing)
...
%   passing :
%       0  not passing any vector between tau
%       1  passing half of the vectors
%       2  passing all the vectors
nVars = length(x);
timePqnProject = 0;
...
i = itn+1;
itnpqn = 1;
while 1;%funEvals <= maxIter

    % Compute Step Direction using the latest corrections gradient
    % infomation
        if itnpqn == 1   %***********notice here******************%
            p = funProj(x-g);
            projects = projects+1;
            if passing == 0% not passing
                ii = itnpqn;
            else 
                ii = i;
            end
            if ii == 1    % all the beginning
                S = zeros(nVars,0);
                Y = zeros(nVars,0);
                Hdiag = 1;
            else
                % pass part of S and Y
                if passing == 1 % pass half the vectors
                    temp = min(corrections,i-2);
                    if size(S,2) == 0 ;
                    else
                        S = S(:,max(end-floor(temp/2),end-1):end);
                        Y = Y(:,max(end-floor(temp/2),end-1):end);
                    end
                else
%                 % use the previous one as an initial guess
%                 S = S;
%                 Y = Y;
                end
            end
    ...
    % Check optimality
    if i == inf; 1;% which will never happen
    else
        if optCond < optTol
