...
% sigma_ref
load infospg
sigma_ref = zeros(20,1);
% for i = 1:length(infospg)
%     sigma_ref(i) = infospg{i}.rNorm;
% end
opts.iterations = 10;
opts.fid = fopen('pqn_only','w');
t = cputime;
RandStream.setDefaultStream(RandStream('mt19937ar','seed',1));
[results_pqnonly,~,~,~,infopqnonly] = MGNFWI(vel1,Dobs,wavelet,model,opts,2,sigma_ref);
t_pqn = cputime - t;
save results_pqnonly results_pqnly
save infopqnonly infopqnonly